    ...
    public void onCommand() {
        ...
        if (args.length==7) {
            ...
            try {
                ...
                sendMessage("please use the format: \n\t"+
                        e.getJDA().getSelfUser().getAsMention()+" can i go eat in [n] hours");
                return;
            }
        } else {
        ...
        switch (e.getAuthor().getId()) {
            case "125857288807251968": //me
                if (Schedule.PIONEER_HALL.canGo(TimeZone.getTimeZone("GMT-5"), hours)) {
                    sendMessage("yes!");
                }
                break;
            case "235275443941539840": //da-CoUGH i mean Satan
                if (Schedule.DLG_COMMONS.canGo(TimeZone.getTimeZone("GMT-7"), hours)) {
                    if (Schedule.CARRILLO_COMMONS.canGo(TimeZone.getTimeZone("GMT-7"), hours)) {
                        sendMessage("yes! both Carrillo and DLG "+(hours==0?"are":(hours>0?"will be":"were"))+" open.");
                    } else {
                        sendMessage("yes! DLG "+(hours==0?"is":(hours>0?"will be":"was"))+" open.");
                    }
                } else if (Schedule.CARRILLO_COMMONS.canGo(TimeZone.getTimeZone("GMT-7"), hours)) {
                    sendMessage("yes! Carrillo "+(hours==0?"is":(hours>0?"will be":"was"))+" open.");
                } else {
