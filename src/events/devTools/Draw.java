...
public class Draw extends MessageListener {
    ...
    public void onCommand() {
        ...
        for (int i=0; i<10; i++) {
            ...
            //but it's fine because y isn't needed until x is valid
            Point p;
            while (coords.contains(
                    p = new Point(
                            ((int)(Math.random()*WIDTH))*SCALE,
                            ((int)(Math.random()*HEIGHT))*SCALE)))
                /*do nothing*/;
            coords.add(p);

            //TODO: make this less stupid when i'm less stupid
            Hero r = HeroDatabase.HEROES.get(
                    (int)(Math.random()*HeroDatabase.HEROES.size()));
            BufferedImage face;
            try {
                face = ImageIO.read(r.getPortraitLink());
            } catch (IOException ioe) {
                new Error("could not read image: " +
                        r.getPortraitLink()).printStackTrace();
                continue;
            }
