...
public class SummonSimulator extends FEHCommand {
    public void onCommand() {
            ...
            if (x.getUser().getId().equals(e.getAuthor().getId())) {
                ...
                if (x.isSummoning()) {
                    ...
                    if (currentSession.canClose()) {
                        ...
                        sendMessage("please summon at least one stone " +
                                "before starting a new session. " +
                                "link: " +
                                currentSession.getSessionMessage()
                                        .getJumpUrl());
                        return;
                    }

                    /* more accurate
                    sendMessage("please close your previous session
        ...
        String report =
                "created new summoning circle for " +
                        e.getAuthor().getName() + "\n\t\t" +
        ...
        for (int i=0; i<banner.getRarityFPool().size(); i++) {
            Hero x = banner.getRarityFPool().get(i);
                //TODO: units like Olwen (same mov/weapon per alt)
                // are not clarified
            ...
            if (i%2==1&&i+1!=banner.getRarityFPool().size())
                featuredUnitsSB.append("\n\t\t\t\t\t\t");
        }
        String featuredUnits =
                featuredUnitsSB.substring(0, featuredUnitsSB.length()-2);
        report+= featuredUnits + "\n\t\t\t\t\t";
        report+= "banner date: " +
                printDate(banner.getStartDate()) +
                " - "+printDate(banner.getEndDate());
