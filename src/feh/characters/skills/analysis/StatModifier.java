...
public interface StatModifier {
    ...
    //deprecated?
    //stat/cooldown modifiers are only evaluated upon the creation of a user's
    //kit, so simply checking for cooldown modifiers on ALL skills does not
    //consume a significant amount of resources. it would probably be easier to
    //process a null modifier result from a few hundred skills than to isolate a
    //handful which do modify stats/cooldown
    ...
    static int[] parseStatModifiers(String description) {
        ...
        if (grantsIndex>=0) {
            ...
            ArrayList<String> args =
                    new ArrayList<>(Arrays.asList(grants.split(" ")));
        ...
        if (inflictsIndex>=0) {
            ...
            ArrayList<String> args =
                    new ArrayList<>(Arrays.asList(inflicts.split(" ")));
