...
public class Stone {
    ...
    private Unit generateUnit() {
        ...
        double  rarityFRate = banner.getRarityFRate(),
                ...
                //not necessary but here for error checking i guess
                rarity3Rate = rarity4Rate+banner.getRarity3Rate();

        if (r<banner.getRarityFRate()) {
            rarity = 5;
            character = banner.getRarityFPool()
                    .get((int)(Math.random()*banner.getRarityFPool().size()));
            //System.out.println(banner.getRarityFPool().size());
        } else if (r<rarity5Rate) {
            rarity = 5;
            character = banner.getRarity5Pool()
                    .get((int)(Math.random()*banner.getRarity5Pool().size()));
            //System.out.println(banner.getRarity5Pool().size());
        } else if (r<rarity4Rate) {
            rarity = 4;
            character = banner.getRarity4Pool()
                    .get((int)(Math.random()*banner.getRarity4Pool().size()));
            //System.out.println(banner.getRarity4Pool().size());
        } else if (r<rarity3Rate) {
            rarity = 3;
            character = banner.getRarity3Pool()
                    .get((int)(Math.random()*banner.getRarity3Pool().size()));
            //System.out.println(banner.getRarity3Pool().size());
        } else {
            System.out.println("Stone ran into an issue: the rarities for " +
                    banner.getName()+" do not add up to 100%");
            throw new Error();
        }

        //Unit class generates random IVs for us
        // TODO: might wanna reorganize this
